...
export const bundleOptimization = {
  ...
  dynamicImport: async <T>(modulePath: string): Promise<T> => {
    try {
      ...
      logger.error('Dynamic import failed', { error: error instanceof Error ? error : String(error), modulePath, errorMessage: error instanceof Error ? error.message : String(error) })
      throw error
    }
  },
